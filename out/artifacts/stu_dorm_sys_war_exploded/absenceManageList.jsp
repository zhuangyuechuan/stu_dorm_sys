					...
					<tr>
						...
						<td>
							...
							<c:if test="${user.disabled == 0 }">
								<button class="btn btn-mini btn-danger" type="button" onclick="userDeleteOrActive(${user.id },1)">修改</button>
							</c:if>
							<c:if test="${user.disabled == 1}">
								<button class="btn btn-mini btn-danger" type="button" onclick="userDeleteOrActive(${user.id },0)">删除</button>
							</c:if>
						</td>
					</tr>
				</c:forEach>
